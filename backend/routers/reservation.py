...
def create_reservation(reservation: ReservationCreate, db: Session = Depends(get_db)):
    validate_room_exists(reservation.room_id, db)
...
def update_reservation(
    ...
    validate_room_exists(reservation_data.room_id, db)
...
def validate_room_exists(room_id: int, db: Session):
    room = db.query(Room).filter(Room.id == room_id).first()
    if not room:
        raise HTTPException(status_code=404, detail="Room not found")


def validate_reservation_duration(time_from: datetime.time, time_to: datetime.time):
